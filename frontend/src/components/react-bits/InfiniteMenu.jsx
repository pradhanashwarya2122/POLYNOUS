// InfiniteMenu (React Bits) adapted for POLYNOUS.
// Change vs source: accepts an `onSelect(item)` callback so the action button
// drives internal navigation instead of window.open, and the overlay text is
// themed via InfiniteMenu.css. WebGL rendering is unchanged.
...
function createProgram(gl, shaderSources, transformFeedbackVaryings, attribLocations) {
  ...
  [gl.VERTEX_SHADER, gl.FRAGMENT_SHADER].forEach((type, ndx) => { const shader = createShader(gl, type, shaderSources[ndx]); if (shader) gl.attachShader(program, shader); });
...
const defaultItems = [{ image: '', link: '', title: '', description: '' }];
...
export default function InfiniteMenu({ items = [], scale = 1.0, onSelect }) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index) => { setActiveItem(list[index % list.length]); };
    if (canvas) {
    ...
    handleResize();
    return () => { window.removeEventListener('resize', handleResize); if (sketch) sketch.dispose(); };
  }, [items, scale]);

  const handleButtonClick = () => {
    if (!activeItem) return;
    if (onSelect) { onSelect(activeItem); return; }
  ...
  return (
    <div className="im-root">
      ...
      {activeItem && (
        <>
          <h2 className={`im-face-title ${isMoving ? 'im-inactive' : 'im-active'}`}>{activeItem.title}</h2>
          <p className={`im-face-description ${isMoving ? 'im-inactive' : 'im-active'}`}>{activeItem.description}</p>
          <button onClick={handleButtonClick} className={`im-action-button ${isMoving ? 'im-inactive' : 'im-active'}`} aria-label="Open">
