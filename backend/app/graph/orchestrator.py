...
def critic_node(state: AgentState) -> AgentState:
    ...
    # Resolve provider the SAME way every other node does. NOTE: 'provider' is
    # NOT a declared AgentState key, so LangGraph strips it between nodes —
    # reading state['provider'] here silently fell back to 'anthropic', making
    # the critic call Anthropic with a non-Anthropic key and fail every run
    # (empty agreement groups → confidence 0). _get_provider reads the declared
